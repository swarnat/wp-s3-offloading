...
use BWPS\SSU\Aws3\Aws\Retry\ConfigurationInterface;
use BWPS\SSU\Aws3\Aws\Retry\QuotaManager;
use BWPS\SSU\Aws3\Aws\Retry\RateLimiter;
use BWPS\SSU\Aws3\Aws\Retry\RetryHelperTrait;
...
class RetryMiddlewareV2
{
    ...
    public static function createDefaultDecider(\BWPS\SSU\Aws3\Aws\Retry\QuotaManager $quotaManager, $maxAttempts = 3, $options = [])
    {
        $retryCurlErrors = [];
        if (extension_loaded('curl')) {
            $retryCurlErrors[CURLE_RECV_ERROR] = true;
        }
        return function ($attempts, \BWPS\SSU\Aws3\Aws\CommandInterface $command, $result) use($options, $quotaManager, $retryCurlErrors, $maxAttempts) {
    ...
    public function __construct(\BWPS\SSU\Aws3\Aws\Retry\ConfigurationInterface $config, callable $handler, $options = [])
    {
        $this->options = $options;
        $this->maxAttempts = $config->getMaxAttempts();
        $this->mode = $config->getMode();
        $this->nextHandler = $handler;
        $this->quotaManager = new \BWPS\SSU\Aws3\Aws\Retry\QuotaManager();
        ...
        if ($this->mode === 'adaptive') {
            $this->rateLimiter = isset($options['rate_limiter']) ? $options['rate_limiter'] : new \BWPS\SSU\Aws3\Aws\Retry\RateLimiter();
        }
    }
        ...
        $callback = function ($value) use($handler, $cmd, $req, $decider, $delayer, &$attempts, &$requestStats, &$monitoringEvents, &$callback) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                if (!$decider($attempts, $cmd, $value)) {
                    return \BWPS\SSU\Aws3\GuzzleHttp\Promise\rejection_for($this->bindStatsToReturn($value, $requestStats));
                }
