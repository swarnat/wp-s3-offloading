...
use Iterator;
use BWPS\SSU\Aws3\Aws\Exception\EventStreamDataException;
...
class EventParsingIterator implements \Iterator
{
    ...
    /** @var AbstractParser */
    private $parser;
    public function __construct(\BWPS\SSU\Aws3\Psr\Http\Message\StreamInterface $stream, \BWPS\SSU\Aws3\Aws\Api\StructureShape $shape, \BWPS\SSU\Aws3\Aws\Api\Parser\AbstractParser $parser)
    {
        $this->decodingIterator = new \BWPS\SSU\Aws3\Aws\Api\Parser\DecodingEventStreamIterator($stream);
        $this->shape = $shape;
        $this->parser = $parser;
    }
    ...
    private function parseEvent(array $event)
    {
        if (!empty($event['headers'][':message-type'])) {
            ...
            if ($event['headers'][':message-type'] !== 'event') {
                throw new \BWPS\SSU\Aws3\Aws\Api\Parser\Exception\ParserException('Failed to parse unknown message type.');
            }
        }
        if (empty($event['headers'][':event-type'])) {
            throw new \BWPS\SSU\Aws3\Aws\Api\Parser\Exception\ParserException('Failed to parse without event type.');
        }
    ...
    {
        throw new \BWPS\SSU\Aws3\Aws\Exception\EventStreamDataException($event['headers'][':error-code'], $event['headers'][':error-message']);
    }
}
