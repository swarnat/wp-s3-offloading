...
class XmlParser
{
    public function parse(\BWPS\SSU\Aws3\Aws\Api\StructureShape $shape, \SimpleXMLElement $value)
    {
    ...
    private function parse_structure(\BWPS\SSU\Aws3\Aws\Api\StructureShape $shape, \SimpleXMLElement $value)
    {
        $target = [];
    ...
    private function memberKey(\BWPS\SSU\Aws3\Aws\Api\Shape $shape, $name)
    {
    ...
    private function parse_list(\BWPS\SSU\Aws3\Aws\Api\ListShape $shape, \SimpleXMLElement $value)
    {
        $target = [];
    ...
    private function parse_map(\BWPS\SSU\Aws3\Aws\Api\MapShape $shape, \SimpleXMLElement $value)
    {
        $target = [];
        if (!$shape['flattened']) {
            $value = $value->entry;
    ...
    private function parse_blob(\BWPS\SSU\Aws3\Aws\Api\Shape $shape, $value)
    {
        return base64_decode((string) $value);
    }
    private function parse_float(\BWPS\SSU\Aws3\Aws\Api\Shape $shape, $value)
    {
        return (double) (string) $value;
    }
    private function parse_integer(\BWPS\SSU\Aws3\Aws\Api\Shape $shape, $value)
    {
        return (int) (string) $value;
    }
    private function parse_boolean(\BWPS\SSU\Aws3\Aws\Api\Shape $shape, $value)
    {
        return $value == 'true';
    }
    private function parse_timestamp(\BWPS\SSU\Aws3\Aws\Api\Shape $shape, $value)
    {
        if (is_string($value) || is_int($value) || is_object($value) && method_exists($value, '__toString')) {
            return \BWPS\SSU\Aws3\Aws\Api\DateTimeResult::fromTimestamp((string) $value, !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : null);
        }
        throw new \BWPS\SSU\Aws3\Aws\Api\Parser\Exception\ParserException('Invalid timestamp value passed to XmlParser::parse_timestamp');
    }
    private function parse_xml_attribute(\BWPS\SSU\Aws3\Aws\Api\Shape $shape, \BWPS\SSU\Aws3\Aws\Api\Shape $memberShape, $value)
    {
