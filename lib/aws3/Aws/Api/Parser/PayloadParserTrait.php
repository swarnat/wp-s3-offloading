...
trait PayloadParserTrait
{
    ...
    private function parseJson($json, $response)
    {
        ...
        if (JSON_ERROR_NONE !== json_last_error()) {
            throw new \BWPS\SSU\Aws3\Aws\Api\Parser\Exception\ParserException('Error parsing JSON: ' . json_last_error_msg(), 0, null, ['response' => $response]);
        }
        return $jsonPayload;
    }
    /**
    ...
    protected function parseXml($xml, $response)
    {
        ...
        try {
            ...
            throw new \BWPS\SSU\Aws3\Aws\Api\Parser\Exception\ParserException("Error parsing XML: {$e->getMessage()}", 0, $e, ['response' => $response]);
        } finally {
