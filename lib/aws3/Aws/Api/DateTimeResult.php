...
class DateTimeResult extends \DateTime implements \JsonSerializable
{
    ...
    public static function fromISO8601($iso8601Timestamp)
    {
        if (is_numeric($iso8601Timestamp) || !is_string($iso8601Timestamp)) {
            throw new \BWPS\SSU\Aws3\Aws\Api\Parser\Exception\ParserException('Invalid timestamp value passed to DateTimeResult::fromISO8601');
        }
        return new \BWPS\SSU\Aws3\Aws\Api\DateTimeResult($iso8601Timestamp);
    }
    /**
    ...
    public static function fromTimestamp($timestamp, $expectedFormat = null)
    {
        ...
        if (!(is_string($timestamp) || is_numeric($timestamp))) {
            throw new \BWPS\SSU\Aws3\Aws\Api\Parser\Exception\ParserException('Invalid timestamp value passed to DateTimeResult::fromTimestamp');
        }
        try {
            if ($expectedFormat == 'iso8601') {
                ...
                if ($expectedFormat == 'unixTimestamp') {
                    ...
                    if (\BWPS\SSU\Aws3\Aws\is_valid_epoch($timestamp)) {
                        return self::fromEpoch($timestamp);
                    }
                }
            }
