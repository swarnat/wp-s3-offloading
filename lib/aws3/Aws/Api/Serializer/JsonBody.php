...
class JsonBody
{
     ...
     * @return string
     */
    public static function getContentType(\BWPS\SSU\Aws3\Aws\Api\Service $service)
    {
    ...
    private function format(\BWPS\SSU\Aws3\Aws\Api\Shape $shape, $value)
    {
        switch ($shape['type']) {
            case 'structure':
                $data = [];
            ...
            case 'timestamp':
                ...
                return \BWPS\SSU\Aws3\Aws\Api\TimestampShape::format($value, $timestampFormat);
            default:
                return $value;
        }
    }
