...
use BWPS\SSU\Aws3\Aws\Api\Validator;
use BWPS\SSU\Aws3\Aws\Api\ApiProvider;
use BWPS\SSU\Aws3\Aws\Api\Service;
use BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use BWPS\SSU\Aws3\Aws\ClientSideMonitoring\Configuration;
use BWPS\SSU\Aws3\Aws\Credentials\Credentials;
use BWPS\SSU\Aws3\Aws\Credentials\CredentialsInterface;
use BWPS\SSU\Aws3\Aws\Endpoint\PartitionEndpointProvider;
use BWPS\SSU\Aws3\Aws\EndpointDiscovery\ConfigurationInterface;
use BWPS\SSU\Aws3\Aws\EndpointDiscovery\ConfigurationProvider;
use BWPS\SSU\Aws3\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware;
use BWPS\SSU\Aws3\Aws\Exception\InvalidRegionException;
use BWPS\SSU\Aws3\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use BWPS\SSU\Aws3\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use BWPS\SSU\Aws3\Aws\Signature\SignatureProvider;
use BWPS\SSU\Aws3\Aws\Endpoint\EndpointProvider;
use BWPS\SSU\Aws3\Aws\Credentials\CredentialProvider;
...
class ClientResolver
{
     ...
     */
    public function resolve(array $args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        $args['config'] = [];
    ...
    public static function _apply_retries($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        // A value of 0 for the config option disables retries
        if ($value) {
            $config = \BWPS\SSU\Aws3\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                // # of retries is 1 less than # of attempts
                $decider = \BWPS\SSU\Aws3\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1);
                $list->appendSign(\BWPS\SSU\Aws3\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
            } else {
                $list->appendSign(\BWPS\SSU\Aws3\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries']]), 'retry');
            }
        }
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            $args['credentials'] = \BWPS\SSU\Aws3\Aws\Credentials\CredentialProvider::fromCredentials($value);
        } elseif (is_array($value) && isset($value['key']) && isset($value['secret'])) {
            $args['credentials'] = \BWPS\SSU\Aws3\Aws\Credentials\CredentialProvider::fromCredentials(new \BWPS\SSU\Aws3\Aws\Credentials\Credentials($value['key'], $value['secret'], isset($value['token']) ? $value['token'] : null, isset($value['expires']) ? $value['expires'] : null));
        } elseif ($value === false) {
            $args['credentials'] = \BWPS\SSU\Aws3\Aws\Credentials\CredentialProvider::fromCredentials(new \BWPS\SSU\Aws3\Aws\Credentials\Credentials('', ''));
            $args['config']['signature_version'] = 'anonymous';
        } elseif ($value instanceof CacheInterface) {
            $args['credentials'] = \BWPS\SSU\Aws3\Aws\Credentials\CredentialProvider::defaultProvider($args);
        } else {
            throw new \InvalidArgumentException('Credentials must be an instance of ' . 'BWPS\\SSU\\Aws3\\Aws\\Credentials\\CredentialsInterface, an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    public static function _default_credential_provider(array $args)
    {
        return \BWPS\SSU\Aws3\Aws\Credentials\CredentialProvider::defaultProvider($args);
    }
    public static function _apply_csm($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        if ($value === false) {
            $value = new \BWPS\SSU\Aws3\Aws\ClientSideMonitoring\Configuration(false, \BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
        $list->appendBuild(\BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallMonitoringMiddleware');
        $list->appendAttempt(\BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallAttemptMonitoringMiddleware');
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        $api = new \BWPS\SSU\Aws3\Aws\Api\Service(\BWPS\SSU\Aws3\Aws\Api\ApiProvider::resolve($value, 'api', $args['service'], $args['version']), $value);
        if (empty($args['config']['signing_name']) && isset($api['metadata']['signingName'])) {
            $args['config']['signing_name'] = $api['metadata']['signingName'];
        }
        $args['api'] = $api;
        $args['parser'] = \BWPS\SSU\Aws3\Aws\Api\Service::createParser($api);
        $args['error_parser'] = \BWPS\SSU\Aws3\Aws\Api\Service::createErrorParser($api->getProtocol(), $api);
    }
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            if (!self::isValidRegion($args['region'])) {
                throw new \BWPS\SSU\Aws3\Aws\Exception\InvalidRegionException('Region must be a valid RFC' . ' host label.');
            }
            // Invoke the endpoint provider and throw if it does not resolve.
            $result = \BWPS\SSU\Aws3\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme'], 'options' => self::getEndpointProviderOptions($args)]);
    ...
    {
        return \BWPS\SSU\Aws3\Aws\EndpointDiscovery\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_serializer($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        $list->prependBuild(\BWPS\SSU\Aws3\Aws\Middleware::requestBuilder($value), 'builder');
    }
    public static function _apply_debug($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        if ($value !== false) {
            $list->interpose(new \BWPS\SSU\Aws3\Aws\TraceMiddleware($value === true ? [] : $value, $args['api']));
        }
    }
    public static function _apply_stats($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        ...
        if ($args['stats']['timer']) {
            $list->prependInit(\BWPS\SSU\Aws3\Aws\Middleware::timer(), 'timer');
        }
    }
    public static function _apply_profile($_, array &$args)
    {
        $args['credentials'] = \BWPS\SSU\Aws3\Aws\Credentials\CredentialProvider::ini($args['profile']);
    }
    public static function _apply_validate($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        if ($value === false) {
            return;
        }
        $validator = $value === true ? new \BWPS\SSU\Aws3\Aws\Api\Validator() : new \BWPS\SSU\Aws3\Aws\Api\Validator($value);
        $list->appendValidate(\BWPS\SSU\Aws3\Aws\Middleware::validation($args['api'], $validator), 'validation');
    }
    public static function _apply_handler($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        $list->setHandler($value);
    }
    public static function _default_handler(array &$args)
    {
        return new \BWPS\SSU\Aws3\Aws\WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        $args['handler'] = new \BWPS\SSU\Aws3\Aws\WrappedHttpHandler($value, $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_user_agent($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        if (!is_array($value)) {
            $value = [$value];
        }
        ...
        if (!ini_get('safe_mode') && function_exists('php_uname') && !in_array('php_uname', $disabledFunctions, true)) {
            $osName = "OS/" . php_uname('s') . '/' . php_uname('r');
            if (!empty($osName)) {
                array_unshift($value, $osName);
            }
        }
        array_unshift($value, 'aws-sdk-php/' . \BWPS\SSU\Aws3\Aws\Sdk::VERSION);
        $args['ua_append'] = $value;
        $list->appendBuild(static function (callable $handler) use($value) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request) use($handler, $value) {
                return $handler($command, $request->withHeader('User-Agent', implode(' ', array_merge($value, $request->getHeader('User-Agent')))));
            };
        });
    }
    public static function _apply_endpoint($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        $args['endpoint'] = $value;
    }
    public static function _apply_idempotency_auto_fill($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        $enabled = false;
        $generator = null;
        if (is_bool($value)) {
        ...
        if ($enabled) {
            $list->prependInit(\BWPS\SSU\Aws3\Aws\IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    public static function _default_endpoint_provider(array $args)
    {
        $options = self::getEndpointProviderOptions($args);
        return \BWPS\SSU\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
    }
    public static function _default_serializer(array $args)
    {
        return \BWPS\SSU\Aws3\Aws\Api\Service::createSerializer($args['api'], $args['endpoint']);
    }
    public static function _default_signature_provider()
    {
        return \BWPS\SSU\Aws3\Aws\Signature\SignatureProvider::defaultProvider();
    }
    public static function _default_signature_version(array &$args)
    {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : call_user_func(\BWPS\SSU\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider(), ['service' => $args['service'], 'region' => $args['region']]);
    ...
    public static function _missing_version(array $args)
    {
        ...
        $versions = \BWPS\SSU\Aws3\Aws\Api\ApiProvider::defaultProvider()->getVersions($service);
