...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'BWPS\\SSU\\Aws3\\Aws\\Exception\\AwsException', $collectStats = false)
    {
    ...
    public function __invoke(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\BWPS\SSU\Aws3\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \BWPS\SSU\Aws3\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request, \BWPS\SSU\Aws3\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \BWPS\SSU\Aws3\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request, \BWPS\SSU\Aws3\Aws\CommandInterface $command, array $stats)
    {
