...
namespace BWPS\SSU\Aws3\Aws\EndpointDiscovery;
...
class EndpointDiscoveryMiddleware
{
    ...
    public function __construct(callable $handler, \BWPS\SSU\Aws3\Aws\AwsClient $client, array $args, $config)
    {
    ...
    public function __invoke(\BWPS\SSU\Aws3\Aws\CommandInterface $cmd, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (isset($op['endpointdiscovery'])) {
            $config = \BWPS\SSU\Aws3\Aws\EndpointDiscovery\ConfigurationProvider::unwrap($this->config);
            $isRequired = !empty($op['endpointdiscovery']['required']);
            if ($isRequired && !$config->isEnabled()) {
                throw new \BWPS\SSU\Aws3\Aws\Exception\UnresolvedEndpointException('This operation ' . 'requires the use of endpoint discovery, but this has ' . 'been disabled in the configuration. Enable endpoint ' . 'discovery or use a different operation.');
            }
            // Continue only if enabled by config
            if ($config->isEnabled()) {
                if (isset($op['endpointoperation'])) {
                    throw new \BWPS\SSU\Aws3\Aws\Exception\UnresolvedEndpointException('This operation is ' . 'contradictorily marked both as using endpoint discovery ' . 'and being the endpoint discovery operation. Please ' . 'verify the accuracy of your model files.');
                }
                ...
                // Check/create cache
                if (!isset(self::$cache)) {
                    self::$cache = new \BWPS\SSU\Aws3\Aws\LruArrayCache($config->getCacheLimit());
                }
                if (empty($endpointList = self::$cache->get($cacheKey))) {
                    $endpointList = new \BWPS\SSU\Aws3\Aws\EndpointDiscovery\EndpointList([]);
                }
    ...
    private function discoverEndpoint($cacheKey, \BWPS\SSU\Aws3\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (isset($result['Endpoints'])) {
            ...
            $endpointList = new \BWPS\SSU\Aws3\Aws\EndpointDiscovery\EndpointList($endpointData);
            self::$cache->set($cacheKey, $endpointList);
            return $endpointList->getEndpoint();
        }
        throw new \BWPS\SSU\Aws3\Aws\Exception\UnresolvedEndpointException('The endpoint discovery operation ' . 'yielded a response that did not contain properly formatted ' . 'endpoint data.');
    }
    private function getCacheKey(\BWPS\SSU\Aws3\Aws\Credentials\CredentialsInterface $creds, \BWPS\SSU\Aws3\Aws\CommandInterface $cmd, array $identifiers)
    {
    ...
    private function getDiscoveryCommand(\BWPS\SSU\Aws3\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (!isset($endpointOperation)) {
            throw new \BWPS\SSU\Aws3\Aws\Exception\UnresolvedEndpointException('This command is set to use ' . 'endpoint discovery, but no endpoint discovery operation was ' . 'found. Please verify the accuracy of your model files.');
        }
        $params = [];
        if (!empty($identifiers)) {
        ...
        $command->getHandlerList()->appendBuild(\BWPS\SSU\Aws3\Aws\Middleware::mapRequest(function (\BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $r) {
    ...
    private function handleDiscoveryException($isRequired, $originalUri, \Exception $e, \BWPS\SSU\Aws3\Aws\CommandInterface $cmd, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($isRequired) {
            ...
            throw new \BWPS\SSU\Aws3\Aws\Exception\AwsException($message, $cmd, ['code' => 'EndpointDiscoveryException', 'message' => $message], $e);
        }
    ...
    private function modifyRequest(\BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request, $endpoint)
    {
    ...
    private function parseEndpoint($endpoint)
    {
        ...
        throw new \BWPS\SSU\Aws3\Aws\Exception\UnresolvedEndpointException("The supplied endpoint '" . "{$endpoint}' is invalid.");
    }
    private function useOriginalUri(\BWPS\SSU\Aws3\Psr\Http\Message\UriInterface $uri, \BWPS\SSU\Aws3\Aws\CommandInterface $cmd, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $nextHandler = $this->nextHandler;
