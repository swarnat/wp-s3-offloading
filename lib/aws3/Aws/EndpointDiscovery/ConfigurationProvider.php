...
use BWPS\SSU\Aws3\Aws\EndpointDiscovery\Exception\ConfigurationException;
...
class ConfigurationProvider extends \BWPS\SSU\Aws3\Aws\AbstractConfigurationProvider implements \BWPS\SSU\Aws3\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \BWPS\SSU\Aws3\Aws\EndpointDiscovery\ConfigurationInterface::class;
    protected static $exceptionClass = \BWPS\SSU\Aws3\Aws\EndpointDiscovery\Exception\ConfigurationException::class;
    /**
    ...
    public static function env($cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        return function () use($cacheLimit) {
            ...
            if ($enabled !== false && $enabled !== '') {
                return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\EndpointDiscovery\Configuration($enabled, $cacheLimit));
            }
    ...
    public static function fallback($config = [])
    {
        ...
        return function () use($enabled) {
            return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\EndpointDiscovery\Configuration($enabled, self::DEFAULT_CACHE_LIMIT));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null, $cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        ...
        return function () use($profile, $filename, $cacheLimit) {
            ...
            return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\EndpointDiscovery\Configuration($data[$profile]['endpoint_discovery_enabled'], $cacheLimit));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof ConfigurationInterface) {
            ...
            if (isset($config['cache_limit'])) {
                return new \BWPS\SSU\Aws3\Aws\EndpointDiscovery\Configuration($config['enabled'], $config['cache_limit']);
            }
            return new \BWPS\SSU\Aws3\Aws\EndpointDiscovery\Configuration($config['enabled'], self::DEFAULT_CACHE_LIMIT);
        }
