...
use BWPS\SSU\Aws3\Aws\Signature\SignatureV4;
...
class PresignUrlMiddleware
{
    ...
    public function __construct(array $options, callable $endpointProvider, \BWPS\SSU\Aws3\Aws\AwsClientInterface $client, callable $nextHandler)
    {
    ...
    public static function wrap(\BWPS\SSU\Aws3\Aws\AwsClientInterface $client, callable $endpointProvider, array $options = [])
    {
        return function (callable $handler) use($endpointProvider, $client, $options) {
            $f = new \BWPS\SSU\Aws3\Aws\PresignUrlMiddleware($options, $endpointProvider, $client, $handler);
            return $f;
        };
    }
    public function __invoke(\BWPS\SSU\Aws3\Aws\CommandInterface $cmd, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function createPresignedUrl(\BWPS\SSU\Aws3\Aws\AwsClientInterface $client, \BWPS\SSU\Aws3\Aws\CommandInterface $cmd)
    {
        $cmdName = $cmd->getName();
        ...
        $request = \BWPS\SSU\Aws3\Aws\serialize($newCmd);
        // Create the new endpoint for the target endpoint.
        $endpoint = \BWPS\SSU\Aws3\Aws\Endpoint\EndpointProvider::resolve($this->endpointProvider, ['region' => $cmd['SourceRegion'], 'service' => $this->serviceName])['endpoint'];
        // Set the request to hit the target endpoint.
        $uri = $request->getUri()->withHost((new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\Uri($endpoint))->getHost());
        $request = $request->withUri($uri);
        // Create a presigned URL for our generated request.
        $signer = new \BWPS\SSU\Aws3\Aws\Signature\SignatureV4($this->serviceName, $cmd['SourceRegion']);
        ...
        return (string) $signer->presign(\BWPS\SSU\Aws3\Aws\Signature\SignatureV4::convertPostToGet($request, $paramsToAdd ?: ""), $client->getCredentials()->wait(), '+1 hour')->getUri();
    }
}
