 ...
 * @deprecated
 */
class KmsMaterialsProvider extends \BWPS\SSU\Aws3\Aws\Crypto\MaterialsProvider implements \BWPS\SSU\Aws3\Aws\Crypto\MaterialsProviderInterface
{
    ...
    public function fromDecryptionEnvelope(\BWPS\SSU\Aws3\Aws\Crypto\MetadataEnvelope $envelope)
    {
        if (empty($envelope[\BWPS\SSU\Aws3\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER])) {
            throw new \RuntimeException('Not able to detect the materials description.');
        }
        $materialsDescription = json_decode($envelope[\BWPS\SSU\Aws3\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], true);
