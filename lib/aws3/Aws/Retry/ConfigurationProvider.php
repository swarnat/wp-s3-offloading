...
namespace BWPS\SSU\Aws3\Aws\Retry;
...
use BWPS\SSU\Aws3\Aws\Retry\Exception\ConfigurationException;
...
class ConfigurationProvider extends \BWPS\SSU\Aws3\Aws\AbstractConfigurationProvider implements \BWPS\SSU\Aws3\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \BWPS\SSU\Aws3\Aws\Retry\ConfigurationInterface::class;
    protected static $exceptionClass = \BWPS\SSU\Aws3\Aws\Retry\Exception\ConfigurationException::class;
    /**
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($mode)) {
                return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\Retry\Configuration($mode, $maxAttempts));
            }
    ...
    {
        return function () {
            return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\Retry\Configuration(self::DEFAULT_MODE, self::DEFAULT_MAX_ATTEMPTS));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\Retry\Configuration($data[$profile][self::INI_MODE], $maxAttempts));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if (is_int($config)) {
            return new \BWPS\SSU\Aws3\Aws\Retry\Configuration('legacy', $config + 1);
        }
        if (is_array($config) && isset($config['mode'])) {
            $maxAttempts = isset($config['max_attempts']) ? $config['max_attempts'] : self::DEFAULT_MAX_ATTEMPTS;
            return new \BWPS\SSU\Aws3\Aws\Retry\Configuration($config['mode'], $maxAttempts);
        }
