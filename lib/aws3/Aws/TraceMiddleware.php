...
class TraceMiddleware
{
     ...
     *   "scrub_auth" is set to true.
     */
    public function __construct(array $config = [], \BWPS\SSU\Aws3\Aws\Api\Service $service = null)
    {
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
    ...
    private function commandArray(\BWPS\SSU\Aws3\Aws\CommandInterface $cmd)
    {
        return ['instance' => spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $this->getRedactedArray($cmd)];
    }
    private function requestArray(\BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\BWPS\SSU\Aws3\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    private function streamStr(\BWPS\SSU\Aws3\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\BWPS\SSU\Aws3\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\BWPS\SSU\Aws3\Aws\CommandInterface $command)
    {
     ...
     * @return array
     */
    private function getRedactedArray(\BWPS\SSU\Aws3\Aws\CommandInterface $cmd)
    {
