...
use BWPS\SSU\Aws3\GuzzleHttp\Psr7\LazyOpenStream;
...
final class Middleware
{
     ...
     * @return callable
     */
    public static function sourceFile(\BWPS\SSU\Aws3\Aws\Api\Service $api, $bodyParameter = 'Body', $sourceParameter = 'SourceFile')
    {
        return function (callable $handler) use($api, $bodyParameter, $sourceParameter) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, $api, $bodyParameter, $sourceParameter) {
                $operation = $api->getOperation($command->getName());
                $source = $command[$sourceParameter];
                if ($source !== null && $operation->getInput()->hasMember($bodyParameter)) {
                    $command[$bodyParameter] = new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\LazyOpenStream($source, 'r');
     ...
     * @return callable
     */
    public static function validation(\BWPS\SSU\Aws3\Aws\Api\Service $api, \BWPS\SSU\Aws3\Aws\Api\Validator $validator = null)
    {
        $validator = $validator ?: new \BWPS\SSU\Aws3\Aws\Api\Validator();
        return function (callable $handler) use($api, $validator) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null) use($api, $validator, $handler) {
                $operation = $api->getOperation($command->getName());
    ...
    public static function requestBuilder(callable $serializer)
    {
        return function (callable $handler) use($serializer) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command) use($serializer, $handler) {
    ...
    public static function signer(callable $credProvider, callable $signatureFunction)
    {
        return function (callable $handler) use($signatureFunction, $credProvider) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request) use($handler, $signatureFunction, $credProvider) {
                $signer = $signatureFunction($command);
                return $credProvider()->then(function (\BWPS\SSU\Aws3\Aws\Credentials\CredentialsInterface $creds) use($handler, $command, $signer, $request) {
    ...
    public static function tap(callable $fn)
    {
        return function (callable $handler) use($fn) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, $fn) {
                $fn($command, $request);
                return $handler($command, $request);
            };
        };
    ...
    {
        $decider = $decider ?: \BWPS\SSU\Aws3\Aws\RetryMiddleware::createDefaultDecider();
        $delay = $delay ?: [\BWPS\SSU\Aws3\Aws\RetryMiddleware::class, 'exponentialDelay'];
        return function (callable $handler) use($decider, $delay, $stats) {
            return new \BWPS\SSU\Aws3\Aws\RetryMiddleware($decider, $delay, $handler, $stats);
        };
    }
    /**
    ...
    public static function invocationId()
    {
        return function (callable $handler) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request) use($handler) {
    ...
    public static function contentType(array $operations)
    {
        return function (callable $handler) use($operations) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, $operations) {
                if (!$request->hasHeader('Content-Type') && in_array($command->getName(), $operations, true) && ($uri = $request->getBody()->getMetadata('uri'))) {
                    $request = $request->withHeader('Content-Type', \BWPS\SSU\Aws3\GuzzleHttp\Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream');
                }
                return $handler($command, $request);
            };
        };
     ...
     * @return callable
     */
    public static function history(\BWPS\SSU\Aws3\Aws\History $history)
    {
        return function (callable $handler) use($history) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, $history) {
                ...
                return $handler($command, $request)->then(function ($result) use($history, $ticket) {
                    ...
                    return \BWPS\SSU\Aws3\GuzzleHttp\Promise\rejection_for($reason);
                });
            };
        };
    }
    ...
    public static function mapRequest(callable $f)
    {
        return function (callable $handler) use($f) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, $f) {
    ...
    public static function timer()
    {
        return function (callable $handler) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\BWPS\SSU\Aws3\Aws\ResultInterface $res) use($start) {
                    ...
                    return \BWPS\SSU\Aws3\GuzzleHttp\Promise\rejection_for($err);
                });
            };
        };
    }
