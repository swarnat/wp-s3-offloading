...
use BWPS\SSU\Aws3\Aws\Api\DocModel;
use BWPS\SSU\Aws3\Aws\Api\Service;
use BWPS\SSU\Aws3\Aws\AwsClient;
use BWPS\SSU\Aws3\Aws\CacheInterface;
use BWPS\SSU\Aws3\Aws\ClientResolver;
use BWPS\SSU\Aws3\Aws\Command;
use BWPS\SSU\Aws3\Aws\Exception\AwsException;
use BWPS\SSU\Aws3\Aws\HandlerList;
use BWPS\SSU\Aws3\Aws\InputValidationMiddleware;
use BWPS\SSU\Aws3\Aws\Middleware;
use BWPS\SSU\Aws3\Aws\Retry\QuotaManager;
use BWPS\SSU\Aws3\Aws\RetryMiddleware;
use BWPS\SSU\Aws3\Aws\ResultInterface;
use BWPS\SSU\Aws3\Aws\CommandInterface;
use BWPS\SSU\Aws3\Aws\RetryMiddlewareV2;
use BWPS\SSU\Aws3\Aws\S3\UseArnRegion\Configuration;
use BWPS\SSU\Aws3\Aws\S3\UseArnRegion\ConfigurationInterface;
use BWPS\SSU\Aws3\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use BWPS\SSU\Aws3\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use BWPS\SSU\Aws3\GuzzleHttp\Exception\RequestException;
use BWPS\SSU\Aws3\GuzzleHttp\Promise\Promise;
use BWPS\SSU\Aws3\GuzzleHttp\Promise\PromiseInterface;
 ...
 */
class S3Client extends \BWPS\SSU\Aws3\Aws\AwsClient implements \BWPS\SSU\Aws3\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var array */
    ...
    public function __construct(array $args)
    {
        if (!isset($args['s3_us_east_1_regional_endpoint']) || $args['s3_us_east_1_regional_endpoint'] instanceof CacheInterface) {
            $args['s3_us_east_1_regional_endpoint'] = \BWPS\SSU\Aws3\Aws\S3\RegionalEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        parent::__construct($args);
        $stack = $this->getHandlerList();
        $stack->appendInit(\BWPS\SSU\Aws3\Aws\S3\SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 's3.ssec');
        $stack->appendBuild(\BWPS\SSU\Aws3\Aws\S3\ApplyChecksumMiddleware::wrap($this->getApi()), 's3.checksum');
        $stack->appendBuild(\BWPS\SSU\Aws3\Aws\Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type');
        // Use the bucket style middleware when using a "bucket_endpoint" (for cnames)
        if ($this->getConfig('bucket_endpoint')) {
            $stack->appendBuild(\BWPS\SSU\Aws3\Aws\S3\BucketEndpointMiddleware::wrap(), 's3.bucket_endpoint');
        } else {
            $stack->appendBuild(\BWPS\SSU\Aws3\Aws\S3\S3EndpointMiddleware::wrap($this->getRegion(), $this->getConfig('endpoint_provider'), ['dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint')]), 's3.endpoint_middleware');
        }

        $stack->appendBuild(\BWPS\SSU\Aws3\Aws\S3\BucketEndpointArnMiddleware::wrap($this->getApi(), $this->getRegion(), ['use_arn_region' => $this->getConfig('use_arn_region'), 'dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint'), 'endpoint' => isset($args['endpoint']) ? $args['endpoint'] : null]), 's3.bucket_endpoint_arn');
        $stack->appendValidate(\BWPS\SSU\Aws3\Aws\InputValidationMiddleware::wrap($this->getApi(), self::$mandatoryAttributes), 'input_validation_middleware');
        $stack->appendSign(\BWPS\SSU\Aws3\Aws\S3\PutObjectUrlMiddleware::wrap(), 's3.put_object_url');
        $stack->appendSign(\BWPS\SSU\Aws3\Aws\S3\PermanentRedirectMiddleware::wrap(), 's3.permanent_redirect');
        $stack->appendInit(\BWPS\SSU\Aws3\Aws\Middleware::sourceFile($this->getApi()), 's3.source_file');
    ...
    public static function _apply_use_arn_region($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        if ($value instanceof CacheInterface) {
            $value = \BWPS\SSU\Aws3\Aws\S3\UseArnRegion\ConfigurationProvider::defaultProvider($args);
        }
        ...
        if ($value instanceof ConfigurationInterface) {
            ...
            $args['use_arn_region'] = new \BWPS\SSU\Aws3\Aws\S3\UseArnRegion\Configuration($value);
        }
    }
    public function createPresignedRequest(\BWPS\SSU\Aws3\Aws\CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(\BWPS\SSU\Aws3\Aws\serialize($command), $this->getCredentials()->wait(), $expires, $options);
    }
    /**
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \BWPS\SSU\Aws3\Aws\serialize($command)->getUri();
    }
    /**
    ...
    private function getLocationConstraintMiddleware()
    {
        ...
        return static function (callable $handler) use($region) {
            return function (\BWPS\SSU\Aws3\Aws\Command $command, $request = null) use($handler, $region) {
    ...
    private function getSaveAsParameter()
    {
        return static function (callable $handler) {
            return function (\BWPS\SSU\Aws3\Aws\Command $command, $request = null) use($handler) {
    ...
    {
        return static function (callable $handler) {
            return function (\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler) {
    ...
    private function getEncodingTypeMiddleware()
    {
        return static function (callable $handler) {
            return function (\BWPS\SSU\Aws3\Aws\Command $command, $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\BWPS\SSU\Aws3\Aws\ResultInterface $result) use($autoSet) {
    ...
    /** @internal */
    public static function _applyRetryConfig($value, $args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \BWPS\SSU\Aws3\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $maxRetries = $config->getMaxAttempts() - 1;
                $decider = \BWPS\SSU\Aws3\Aws\RetryMiddleware::createDefaultDecider($maxRetries);
                ...
                $delay = [\BWPS\SSU\Aws3\Aws\RetryMiddleware::class, 'exponentialDelay'];
                $list->appendSign(\BWPS\SSU\Aws3\Aws\Middleware::retry($decider, $delay), 'retry');
            } else {
                $defaultDecider = \BWPS\SSU\Aws3\Aws\RetryMiddlewareV2::createDefaultDecider(new \BWPS\SSU\Aws3\Aws\Retry\QuotaManager(), $config->getMaxAttempts());
                $list->appendSign(\BWPS\SSU\Aws3\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'decider' => function ($attempts, \BWPS\SSU\Aws3\Aws\CommandInterface $cmd, $result) use($defaultDecider, $config) {
    ...
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \BWPS\SSU\Aws3\Aws\HandlerList $list)
    {
        \BWPS\SSU\Aws3\Aws\ClientResolver::_apply_api_provider($value, $args);
        $args['parser'] = new \BWPS\SSU\Aws3\Aws\S3\GetBucketLocationParser(new \BWPS\SSU\Aws3\Aws\S3\AmbiguousSuccessParser(new \BWPS\SSU\Aws3\Aws\S3\RetryableMalformedResponseParser($args['parser'], $args['exception_class']), $args['error_parser'], $args['exception_class']));
    }
    /**
     * @internal
     * @codeCoverageIgnore
    ...
    public static function applyDocFilters(array $api, array $docs)
    {
        ...
        return [new \BWPS\SSU\Aws3\Aws\Api\Service($api, \BWPS\SSU\Aws3\Aws\Api\ApiProvider::defaultProvider()), new \BWPS\SSU\Aws3\Aws\Api\DocModel($docs)];
    }
    /**
     * @internal
     * @codeCoverageIgnore
