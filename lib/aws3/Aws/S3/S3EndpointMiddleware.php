...
use BWPS\SSU\Aws3\GuzzleHttp\Exception\InvalidArgumentException;
use BWPS\SSU\Aws3\GuzzleHttp\Psr7\Uri;
...
class S3EndpointMiddleware
{
    ...
    public function __construct(callable $nextHandler, $region, array $options, $endpointProvider = null)
    {
        ...
        $this->endpointProvider = is_null($endpointProvider) ? \BWPS\SSU\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider() : $endpointProvider;
    ...
    private static function isRequestHostStyleCompatible(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        return \BWPS\SSU\Aws3\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']) && ($request->getUri()->getScheme() === 'http' || strpos($command['Bucket'], '.') === false) && filter_var($request->getUri()->getHost(), FILTER_VALIDATE_IP) === false;
    }
    private function endpointPatternDecider(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function canAccelerate(\BWPS\SSU\Aws3\Aws\CommandInterface $command)
    {
        return empty(self::$exclusions[$command->getName()]) && \BWPS\SSU\Aws3\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']);
    }
    private function getBucketStyleHost(\BWPS\SSU\Aws3\Aws\CommandInterface $command, $host)
    {
    ...
    private function applyHostStyleEndpoint(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $uri = $request->getUri();
    ...
    private function applyPathStyleEndpointCustomizations(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function applyDualStackEndpoint(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function applyAccelerateEndpoint(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request, $pattern)
    {
    ...
    private function getAccelerateHost(\BWPS\SSU\Aws3\Aws\CommandInterface $command, $pattern)
    {
        $dnsSuffix = $this->endpointProvider->getPartition($this->region, 's3')->getDnsSuffix();
        return "{$command['Bucket']}.{$pattern}.{$dnsSuffix}";
    }
    private function getBucketlessPath($path, \BWPS\SSU\Aws3\Aws\CommandInterface $command)
    {
        $pattern = '/^\\/' . preg_quote($command['Bucket'], '/') . '/';
        return preg_replace($pattern, '', $path) ?: '/';
    }
    private function applyEndpoint(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $dualStack = isset($command['@use_dual_stack_endpoint']) ? $command['@use_dual_stack_endpoint'] : $this->dualStackByDefault;
        if (\BWPS\SSU\Aws3\Aws\Arn\ArnParser::isArn($command['Bucket'])) {
            $arn = \BWPS\SSU\Aws3\Aws\Arn\ArnParser::parse($command['Bucket']);
            $outpost = $arn->getService() == 's3-outposts';
            if ($outpost && $dualStack) {
                throw new \BWPS\SSU\Aws3\GuzzleHttp\Exception\InvalidArgumentException("Outposts + dualstack is not supported");
            }
        ...
        if ($dualStack) {
            throw new \BWPS\SSU\Aws3\GuzzleHttp\Exception\InvalidArgumentException("Custom Endpoint + Dualstack not supported");
        }
        if ($command->getName() == 'WriteGetObjectResponse') {
