...
use BWPS\SSU\Aws3\Aws\Multipart\AbstractUploadManager;
use BWPS\SSU\Aws3\Aws\ResultInterface;
use BWPS\SSU\Aws3\GuzzleHttp\Psr7;
class MultipartCopy extends \BWPS\SSU\Aws3\Aws\Multipart\AbstractUploadManager
{
    ...
    {
        if (\BWPS\SSU\Aws3\Aws\Arn\ArnParser::isArn($source)) {
            $this->source = '';
        } else {
            $this->source = "/";
        }
    ...
    private function fetchSourceMetadata()
    {
        ...
        if (strpos($key, '?')) {
            ...
            $headParams['Key'] = $key;
            $query = \BWPS\SSU\Aws3\GuzzleHttp\Psr7\parse_query($query, false);
