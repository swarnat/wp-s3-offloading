 ...
 * </code>
 */
class S3EncryptionClientV2 extends \BWPS\SSU\Aws3\Aws\Crypto\AbstractCryptoClientV2
{
     ...
     */
    public function __construct(\BWPS\SSU\Aws3\Aws\S3\S3Client $client, $instructionFileSuffix = null)
    {
    ...
    public function putObjectAsync(array $args)
    {
        ...
        return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for($this->encrypt(\BWPS\SSU\Aws3\GuzzleHttp\Psr7\stream_for($args['Body']), $args, $provider, $envelope))->then(function ($encryptedBodyStream) use($args) {
            $hash = new \BWPS\SSU\Aws3\Aws\PhpHash('sha256');
            $hashingEncryptedBodyStream = new \BWPS\SSU\Aws3\Aws\HashingStream($encryptedBodyStream, $hash, self::getContentShaDecorator($args));
    ...
    public function getObjectAsync(array $args)
    {
        ...
        if (!isset($args['@SecurityProfile']) || !in_array($args['@SecurityProfile'], self::$supportedSecurityProfiles)) {
            throw new \BWPS\SSU\Aws3\Aws\Exception\CryptoException("@SecurityProfile is required and must be" . " set to 'V2' or 'V2_AND_LEGACY'");
        }
