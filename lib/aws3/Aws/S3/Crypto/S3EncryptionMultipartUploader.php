...
use BWPS\SSU\Aws3\Aws\S3\MultipartUploader;
use BWPS\SSU\Aws3\Aws\S3\S3ClientInterface;
 ...
 * @deprecated
 */
class S3EncryptionMultipartUploader extends \BWPS\SSU\Aws3\Aws\S3\MultipartUploader
{
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for($this->encrypt($this->source, $this->config['@cipheroptions'] ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
