...
class BucketEndpointMiddleware
{
    ...
    private function modifyRequest(\BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request, \BWPS\SSU\Aws3\Aws\CommandInterface $command)
    {
        $uri = $request->getUri();
        $path = $uri->getPath();
        $bucket = $command['Bucket'];
        ...
        if ($command['Key']) {
            $path = \BWPS\SSU\Aws3\Aws\S3\S3Client::encodeKey(rawurldecode($path));
        }
