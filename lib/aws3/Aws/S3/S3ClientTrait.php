...
use BWPS\SSU\Aws3\GuzzleHttp\Promise\RejectedPromise;
...
trait S3ClientTrait
{
    ...
    {
        return (new \BWPS\SSU\Aws3\Aws\S3\ObjectUploader($this, $bucket, $key, $body, $acl, $options))->promise();
    }
    /**
    ...
    public function copyAsync($fromB, $fromK, $destB, $destK, $acl = 'private', array $opts = [])
    {
        ...
        return (new \BWPS\SSU\Aws3\Aws\S3\ObjectCopier($this, $source, $destination, $acl, $opts))->promise();
    }
    /**
    ...
    {
        \BWPS\SSU\Aws3\Aws\S3\StreamWrapper::register($this);
    }
    /**
    ...
    {
        if (!$prefix && !$regex) {
            return new \BWPS\SSU\Aws3\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException('A prefix or regex is required.'));
        }
        $params = ['Bucket' => $bucket, 'Prefix' => $prefix];
        $iter = $this->getIterator('ListObjects', $params);
        if ($regex) {
            $iter = \BWPS\SSU\Aws3\Aws\filter($iter, function ($c) use($regex) {
                return preg_match($regex, $c['Key']);
            });
        }
        return \BWPS\SSU\Aws3\Aws\S3\BatchDelete::fromIterator($this, $bucket, $iter, $options)->promise();
    }
    /**
    ...
    public function uploadDirectoryAsync($directory, $bucket, $keyPrefix = null, array $options = [])
    {
        ...
        return (new \BWPS\SSU\Aws3\Aws\S3\Transfer($this, $directory, $d, $options))->promise();
    }
    /**
    ...
    public function downloadBucketAsync($directory, $bucket, $keyPrefix = '', array $options = [])
    {
        ...
        return (new \BWPS\SSU\Aws3\Aws\S3\Transfer($this, $s, $directory, $options))->promise();
    }
    /**
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        return $handler($command)->then(static function (\BWPS\SSU\Aws3\Aws\ResultInterface $result) {
            return $result['@metadata']['headers']['x-amz-bucket-region'];
        }, function (\BWPS\SSU\Aws3\Aws\Exception\AwsException $e) {
    ...
    private function determineBucketRegionFromExceptionBody(\BWPS\SSU\Aws3\Psr\Http\Message\ResponseInterface $response)
    {
        try {
     ...
     */
    private function checkExistenceWithCommand(\BWPS\SSU\Aws3\Aws\CommandInterface $command)
    {
        try {
     ...
     */
    public abstract function execute(\BWPS\SSU\Aws3\Aws\CommandInterface $command);
    /**
