...
use BWPS\SSU\Aws3\Aws\Multipart\UploadState;
...
trait MultipartUploadingTrait
{
     ...
     * @return UploadState
     */
    public static function getStateFromService(\BWPS\SSU\Aws3\Aws\S3\S3ClientInterface $client, $bucket, $key, $uploadId)
    {
        $state = new \BWPS\SSU\Aws3\Aws\Multipart\UploadState(['Bucket' => $bucket, 'Key' => $key, 'UploadId' => $uploadId]);
        ...
        $state->setStatus(\BWPS\SSU\Aws3\Aws\Multipart\UploadState::INITIATED);
        return $state;
    }
    protected function handleResult(\BWPS\SSU\Aws3\Aws\CommandInterface $command, \BWPS\SSU\Aws3\Aws\ResultInterface $result)
    {
        $this->getState()->markPartAsUploaded($command['PartNumber'], ['PartNumber' => $command['PartNumber'], 'ETag' => $this->extractETag($result)]);
    }
    protected abstract function extractETag(\BWPS\SSU\Aws3\Aws\ResultInterface $result);
    ...
    protected function determinePartSize()
    {
        ...
        $partSize = $this->getConfig()['part_size'] ?: \BWPS\SSU\Aws3\Aws\S3\MultipartUploader::PART_MIN_SIZE;
        // Adjust the part size to be larger for known, x-large uploads.
        if ($sourceSize = $this->getSourceSize()) {
            $partSize = (int) max($partSize, ceil($sourceSize / \BWPS\SSU\Aws3\Aws\S3\MultipartUploader::PART_MAX_NUM));
        }
        // Ensure that the part size follows the rules: 5 MB <= size <= 5 GB.
        if ($partSize < \BWPS\SSU\Aws3\Aws\S3\MultipartUploader::PART_MIN_SIZE || $partSize > \BWPS\SSU\Aws3\Aws\S3\MultipartUploader::PART_MAX_SIZE) {
