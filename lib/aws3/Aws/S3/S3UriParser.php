...
class S3UriParser
{
    ...
    public function parse($uri)
    {
        ...
        if (!empty($components)) {
            if (\BWPS\SSU\Aws3\Aws\Arn\ArnParser::isArn($components['host'])) {
                $arn = new \BWPS\SSU\Aws3\Aws\Arn\S3\AccessPointArn($components['host']);
                return ['bucket' => $components['host'], 'key' => $components['path'], 'path_style' => false, 'region' => $arn->getRegion()];
            }
        }
        $url = \BWPS\SSU\Aws3\GuzzleHttp\Psr7\uri_for($uri);
    ...
    private function parseStreamWrapper(\BWPS\SSU\Aws3\Psr\Http\Message\UriInterface $url)
    {
    ...
    private function parseCustomEndpoint(\BWPS\SSU\Aws3\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
    ...
    private function parsePathStyle(\BWPS\SSU\Aws3\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
        if ($url->getPath() != '/') {
            $path = ltrim($url->getPath(), '/');
    ...
    private function parseVirtualHosted(\BWPS\SSU\Aws3\Psr\Http\Message\UriInterface $url, array $matches)
    {
        $result = self::$defaultResult;
        $result['path_style'] = false;
