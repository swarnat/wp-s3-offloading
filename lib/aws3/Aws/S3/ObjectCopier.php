...
use BWPS\SSU\Aws3\Aws\Arn\S3\AccessPointArn;
 ...
 * when appropriate.
 */
class ObjectCopier implements \BWPS\SSU\Aws3\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_MULTIPART_THRESHOLD = \BWPS\SSU\Aws3\Aws\S3\MultipartUploader::PART_MAX_SIZE;
    private $client;
    private $source;
    private $destination;
    private $acl;
     ...
     */
    public function __construct(\BWPS\SSU\Aws3\Aws\S3\S3ClientInterface $client, array $source, array $destination, $acl = 'private', array $options = [])
    {
    ...
    public function promise()
    {
        return \BWPS\SSU\Aws3\GuzzleHttp\Promise\coroutine(function () {
            ...
            if ($objectStats['ContentLength'] > $this->options['mup_threshold']) {
                $mup = new \BWPS\SSU\Aws3\Aws\S3\MultipartCopy($this->client, $this->getSourcePath(), ['source_metadata' => $objectStats, 'acl' => $this->acl] + $this->destination + $this->options);
                (yield $mup->promise());
            } else {
    ...
    {
        if (\BWPS\SSU\Aws3\Aws\Arn\ArnParser::isArn($this->source['Bucket'])) {
            try {
                new \BWPS\SSU\Aws3\Aws\Arn\S3\AccessPointArn($this->source['Bucket']);
            } catch (\Exception $e) {
