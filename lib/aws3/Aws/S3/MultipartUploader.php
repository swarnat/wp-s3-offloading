...
use BWPS\SSU\Aws3\Aws\HashingStream;
use BWPS\SSU\Aws3\Aws\Multipart\AbstractUploader;
use BWPS\SSU\Aws3\Aws\PhpHash;
use BWPS\SSU\Aws3\Aws\ResultInterface;
use BWPS\SSU\Aws3\GuzzleHttp\Psr7;
use BWPS\SSU\Aws3\Psr\Http\Message\StreamInterface as Stream;
use BWPS\SSU\Aws3\Aws\S3\Exception\S3MultipartUploadException;
/**
 * Encapsulates the execution of a multipart upload to S3 or Glacier.
 */
class MultipartUploader extends \BWPS\SSU\Aws3\Aws\Multipart\AbstractUploader
{
     ...
     */
    public function __construct(\BWPS\SSU\Aws3\Aws\S3\S3ClientInterface $client, $source, array $config = [])
    {
        parent::__construct($client, $source, array_change_key_case($config) + ['bucket' => null, 'key' => null, 'exception_class' => \BWPS\SSU\Aws3\Aws\S3\Exception\S3MultipartUploadException::class]);
    }
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = $this->limitPartStream(new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\LazyOpenStream($this->source->getMetadata('uri'), 'r'));
        } else {
            ...
            $body = \BWPS\SSU\Aws3\GuzzleHttp\Psr7\stream_for();
            \BWPS\SSU\Aws3\GuzzleHttp\Psr7\copy_to_stream($source, $body);
        }
    ...
    protected function extractETag(\BWPS\SSU\Aws3\Aws\ResultInterface $result)
    {
        return $result['ETag'];
    }
    protected function getSourceMimeType()
    {
        if ($uri = $this->source->getMetadata('uri')) {
            return \BWPS\SSU\Aws3\GuzzleHttp\Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream';
        }
    }
     ...
     * @return Stream
     */
    private function decorateWithHashes(\BWPS\SSU\Aws3\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        // Decorate source with a hashing stream
        $hash = new \BWPS\SSU\Aws3\Aws\PhpHash('sha256');
        return new \BWPS\SSU\Aws3\Aws\HashingStream($stream, $hash, function ($result) use(&$data) {
