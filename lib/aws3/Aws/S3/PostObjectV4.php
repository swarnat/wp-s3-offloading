...
use BWPS\SSU\Aws3\Aws\Signature\SignatureTrait;
...
class PostObjectV4
{
     ...
     */
    public function __construct(\BWPS\SSU\Aws3\Aws\S3\S3ClientInterface $client, $bucket, array $formInputs, array $options = [], $expiration = '+1 hours')
    {
        $this->client = $client;
        $this->bucket = $bucket;
        ...
        // setup basic policy
        $policy = ['expiration' => \BWPS\SSU\Aws3\Aws\Api\TimestampShape::format($expiration, 'iso8601'), 'conditions' => $options];
    ...
    {
        $uri = new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\Uri($this->client->getEndpoint());
    ...
    protected function getPolicyAndSignature(\BWPS\SSU\Aws3\Aws\Credentials\CredentialsInterface $credentials, array $policy)
    {
        $ldt = gmdate(\BWPS\SSU\Aws3\Aws\Signature\SignatureV4::ISO8601_BASIC);
        $sdt = substr($ldt, 0, 8);
