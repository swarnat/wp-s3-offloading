 ...
 * appropriate.
 */
class ObjectUploader implements \BWPS\SSU\Aws3\GuzzleHttp\Promise\PromisorInterface
{
     ...
     *                                          the sub command(s).
     */
    public function __construct(\BWPS\SSU\Aws3\Aws\S3\S3ClientInterface $client, $bucket, $key, $body, $acl = 'private', array $options = [])
    {
        $this->client = $client;
        $this->bucket = $bucket;
        $this->key = $key;
        $this->body = \BWPS\SSU\Aws3\GuzzleHttp\Psr7\stream_for($body);
        $this->acl = $acl;
    ...
    public function promise()
    {
        ...
        if ($this->requiresMultipart($this->body, $mup_threshold)) {
            ...
            return (new \BWPS\SSU\Aws3\Aws\S3\MultipartUploader($this->client, $this->body, ['bucket' => $this->bucket, 'key' => $this->key, 'acl' => $this->acl] + $this->options))->promise();
        }
     ...
     * @return bool
     */
    private function requiresMultipart(\BWPS\SSU\Aws3\Psr\Http\Message\StreamInterface &$body, $threshold)
    {
         ...
         */
        $buffer = \BWPS\SSU\Aws3\GuzzleHttp\Psr7\stream_for();
        \BWPS\SSU\Aws3\GuzzleHttp\Psr7\copy_to_stream($body, $buffer, \BWPS\SSU\Aws3\Aws\S3\MultipartUploader::PART_MIN_SIZE);
        // If body < 5MB, use PutObject with the buffer.
        if ($buffer->getSize() < \BWPS\SSU\Aws3\Aws\S3\MultipartUploader::PART_MIN_SIZE) {
            $buffer->seek(0);
            $body = $buffer;
            return false;
        }
        ...
        if ($body->isSeekable() && $body->getMetadata('uri') !== 'php://input') {
            ...
            // original stream.
            $buffer->seek(0);
            $body = new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\AppendStream([$buffer, $body]);
        }
        return true;
    }
}
