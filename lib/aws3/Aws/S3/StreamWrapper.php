...
use BWPS\SSU\Aws3\GuzzleHttp\Psr7\CachingStream;
...
class StreamWrapper
{
     ...
     */
    public static function register(\BWPS\SSU\Aws3\Aws\S3\S3ClientInterface $client, $protocol = 's3', \BWPS\SSU\Aws3\Aws\CacheInterface $cache = null)
    {
        ...
        if ($cache) {
            ...
            $default[$protocol]['cache'] = new \BWPS\SSU\Aws3\Aws\LruArrayCache();
        }
    ...
    public function stream_flush()
    {
        ...
        if (!isset($params['ContentType']) && ($type = \BWPS\SSU\Aws3\GuzzleHttp\Psr7\mimetype_from_filename($params['Key']))) {
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $this->objectIterator = \BWPS\SSU\Aws3\Aws\flatmap($this->getClient()->getPaginator('ListObjects', $op), function (\BWPS\SSU\Aws3\Aws\Result $result) use($filterFn) {
    ...
    private function openReadStream()
    {
        ...
        if ($this->getOption('seekable') && !$this->body->isSeekable()) {
            $this->body = new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\CachingStream($this->body);
        }
        return true;
    }
    private function openWriteStream()
    {
        $this->body = new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\Stream(fopen('php://temp', 'r+'));
        return true;
    }
    ...
    {
        if (!$this->cache) {
            $this->cache = $this->getOption('cache') ?: new \BWPS\SSU\Aws3\Aws\LruArrayCache();
        }
        return $this->cache;
    }
    /**
