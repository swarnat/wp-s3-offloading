...
use BWPS\SSU\Aws3\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \BWPS\SSU\Aws3\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
     ...
     */
    public static function convertPostToGet(\BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request, $additionalQueryParams = "")
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\BWPS\SSU\Aws3\GuzzleHttp\Psr7\stream_for(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \BWPS\SSU\Aws3\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \BWPS\SSU\Aws3\GuzzleHttp\Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \BWPS\SSU\Aws3\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    ...
    private function createPresignedRequest(\BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request, \BWPS\SSU\Aws3\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \BWPS\SSU\Aws3\GuzzleHttp\Psr7\parse_query($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\BWPS\SSU\Aws3\GuzzleHttp\Psr7\build_query($req['query']));
        }
        return new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
}
