...
abstract class AbstractUploader extends \BWPS\SSU\Aws3\Aws\Multipart\AbstractUploadManager
{
     ...
     * @param array  $config
     */
    public function __construct(\BWPS\SSU\Aws3\Aws\AwsClientInterface $client, $source, array $config = [])
    {
     ...
     */
    protected function limitPartStream(\BWPS\SSU\Aws3\Psr\Http\Message\StreamInterface $stream)
    {
        // Limit what is read from the stream to the part size.
        return new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\LimitStream($stream, $this->state->getPartSize(), $this->source->tell());
    }
        ...
        for ($partNumber = 1; $this->isEof($seekable); $partNumber++) {
            ...
            if (!$this->state->hasPartBeenUploaded($partNumber)) {
                ...
                if (isset($numberOfParts) && $partNumber > $numberOfParts) {
                    throw new $this->config['exception_class']($this->state, new \BWPS\SSU\Aws3\Aws\Exception\AwsException("Maximum part number for this job exceeded, file has likely been corrupted." . "  Please restart this upload.", $command));
                }
                (yield $command);
    ...
    private function determineSource($source)
    {
        ...
        if (is_string($source)) {
            $source = \BWPS\SSU\Aws3\GuzzleHttp\Psr7\try_fopen($source, 'r');
        }
        // Create a source stream.
        $stream = \BWPS\SSU\Aws3\GuzzleHttp\Psr7\stream_for($source);
