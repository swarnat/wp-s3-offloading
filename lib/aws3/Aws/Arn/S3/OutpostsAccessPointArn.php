 ...
 * @internal
 */
class OutpostsAccessPointArn extends \BWPS\SSU\Aws3\Aws\Arn\AccessPointArn implements \BWPS\SSU\Aws3\Aws\Arn\AccessPointArnInterface, \BWPS\SSU\Aws3\Aws\Arn\S3\OutpostsArnInterface
{
    ...
    {
        \BWPS\SSU\Aws3\Aws\Arn\Arn::validate($data);
        if ($data['service'] !== 's3-outposts') {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 3rd component of an S3 Outposts" . " access point ARN represents the service and must be" . " 's3-outposts'.");
        }
        ...
        if ($data['resource_type'] !== 'outpost') {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 6th component of an S3 Outposts" . " access point ARN represents the resource type and must be" . " 'outpost'.");
        }
        if (!self::isValidHostLabel($data['outpost_id'])) {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 7th component of an S3 Outposts" . " access point ARN is required, represents the outpost ID, and" . " must be a valid host label.");
        }
        if ($data['accesspoint_type'] !== 'accesspoint') {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 8th component of an S3 Outposts" . " access point ARN must be 'accesspoint'");
        }
        if (!self::isValidHostLabel($data['accesspoint_name'])) {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 9th component of an S3 Outposts" . " access point ARN is required, represents the accesspoint name," . " and must be a valid host label.");
        }
        if (!empty($data['resource_extra'])) {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("An S3 Outposts access point ARN" . " should only have 9 components, delimited by the characters" . " ':' and '/'. '{$data['resource_extra']}' was found after the" . " 9th component.");
        }
    }
}
