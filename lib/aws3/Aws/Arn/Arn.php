 ...
 * @internal
 */
class Arn implements \BWPS\SSU\Aws3\Aws\Arn\ArnInterface
{
    protected $data;
    protected $string;
    ...
    public function __construct($data)
    {
        if (is_array($data)) {
            ...
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException('Constructor accepts a string or an' . ' array as an argument.');
        }
    ...
    {
        if ($data['arn'] !== 'arn') {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 1st component of an ARN must be" . " 'arn'.");
        }
        if (empty($data['partition'])) {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 2nd component of an ARN" . " represents the partition and must not be empty.");
        }
        if (empty($data['service'])) {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 3rd component of an ARN" . " represents the service and must not be empty.");
        }
        if (empty($data['resource'])) {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 6th component of an ARN" . " represents the resource information and must not be empty." . " Individual service ARNs may include additional delimiters" . " to further qualify resources.");
        }
    }
    protected static function validateAccountId($data, $arnName)
    {
        if (!self::isValidHostLabel($data['account_id'])) {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 5th component of a {$arnName}" . " is required, represents the account ID, and" . " must be a valid host label.");
        }
    }
    protected static function validateRegion($data, $arnName)
    {
        if (empty($data['region'])) {
            throw new \BWPS\SSU\Aws3\Aws\Arn\Exception\InvalidArnException("The 4th component of a {$arnName}" . " represents the region and must not be empty.");
        }
    }
    /**
