 ...
 */
class CommandPool implements \BWPS\SSU\Aws3\GuzzleHttp\Promise\PromisorInterface
{
    /** @var EachPromise */
    private $each;
    /**
     ...
     */
    public function __construct(\BWPS\SSU\Aws3\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        ...
        $this->each = new \BWPS\SSU\Aws3\GuzzleHttp\Promise\EachPromise($mapFn($commands), $config);
    }
    /**
     ...
     */
    public static function batch(\BWPS\SSU\Aws3\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        $results = [];
