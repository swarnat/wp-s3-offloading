...
namespace BWPS\SSU\Aws3\Aws\Credentials;

use Aws;
use BWPS\SSU\Aws3\Aws\Api\DateTimeResult;
use BWPS\SSU\Aws3\Aws\CacheInterface;
use BWPS\SSU\Aws3\Aws\Exception\CredentialsException;
use BWPS\SSU\Aws3\Aws\Sts\StsClient;
...
class CredentialProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        $shouldUseEcsCredentialsProvider = getenv(\BWPS\SSU\Aws3\Aws\Credentials\EcsCredentialProvider::ENV_URI);
        // getenv() is not thread safe - fall back to $_SERVER
        if ($shouldUseEcsCredentialsProvider === false) {
            $shouldUseEcsCredentialsProvider = isset($_SERVER[\BWPS\SSU\Aws3\Aws\Credentials\EcsCredentialProvider::ENV_URI]) ? $_SERVER[\BWPS\SSU\Aws3\Aws\Credentials\EcsCredentialProvider::ENV_URI] : false;
        }
     ...
     * @return callable
     */
    public static function fromCredentials(\BWPS\SSU\Aws3\Aws\Credentials\CredentialsInterface $creds)
    {
        $promise = \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for($creds);
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\BWPS\SSU\Aws3\Aws\Credentials\CredentialsInterface $creds) use($provider, &$isConstant, &$result) {
                ...
                $result = null;
                return new \BWPS\SSU\Aws3\GuzzleHttp\Promise\RejectedPromise($reason);
            });
        };
    }
    /**
     ...
     * @return callable
     */
    public static function cache(callable $provider, \BWPS\SSU\Aws3\Aws\CacheInterface $cache, $cacheKey = null)
    {
        $cacheKey = $cacheKey ?: 'aws_cached_credentials';
        return function () use($provider, $cache, $cacheKey) {
            $found = $cache->get($cacheKey);
            if ($found instanceof CredentialsInterface && !$found->isExpired()) {
                return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for($found);
            }
            return $provider()->then(function (\BWPS\SSU\Aws3\Aws\Credentials\CredentialsInterface $creds) use($cache, $cacheKey) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($key && $secret) {
                return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\Credentials\Credentials($key, $secret, getenv(self::ENV_SESSION) ?: NULL));
            }
    ...
    {
        return new \BWPS\SSU\Aws3\Aws\Credentials\InstanceProfileProvider($config);
    }
    /**
    ...
    public static function sso($ssoProfileName, $filename = null, $config = [])
    {
        ...
        return function () use($ssoProfileName, $filename, $config) {
            ...
            try {
                $expiration = (new \BWPS\SSU\Aws3\Aws\Api\DateTimeResult($tokenData['expiresAt']))->getTimestamp();
            } catch (\Exception $e) {
            ...
            if (empty($config['ssoClient'])) {
                $ssoClient = new \BWPS\SSU\Aws3\Aws\SSO\SSOClient(['region' => $ssoProfile['sso_region'], 'version' => '2019-06-10', 'credentials' => false]);
            } else {
            ...
            return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\Credentials\Credentials($ssoCredentials['accessKeyId'], $ssoCredentials['secretAccessKey'], $ssoCredentials['sessionToken'], $expiration));
        };
    }
    /**
    ...
    {
        return new \BWPS\SSU\Aws3\Aws\Credentials\EcsCredentialProvider($config);
    }
    /**
    ...
    {
        return new \BWPS\SSU\Aws3\Aws\Credentials\AssumeRoleCredentialProvider($config);
    }
    /**
    ...
    public static function assumeRoleWithWebIdentityCredentialProvider(array $config = [])
    {
        return function () use($config) {
            ...
            if ($tokenFromEnv && $arnFromEnv) {
                ...
                $provider = new \BWPS\SSU\Aws3\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $arnFromEnv, 'WebIdentityTokenFile' => $tokenFromEnv, 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                return $provider();
            }
            ...
            if (isset($profiles[$profileName])) {
                ...
                if (isset($profile['web_identity_token_file']) && isset($profile['role_arn'])) {
                    ...
                    $provider = new \BWPS\SSU\Aws3\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $profile['role_arn'], 'WebIdentityTokenFile' => $profile['web_identity_token_file'], 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                    return $provider();
                }
            } else {
    ...
    public static function ini($profile = null, $filename = null, array $config = [])
    {
        ...
        return function () use($profile, $filename, $config) {
            ...
            */
            ...
            return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\Credentials\Credentials($data[$profile]['aws_access_key_id'], $data[$profile]['aws_secret_access_key'], $data[$profile]['aws_session_token']));
        };
    }
    /**
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \BWPS\SSU\Aws3\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
            ...
            if (isset($processData['Expiration'])) {
                try {
                    $expiration = new \BWPS\SSU\Aws3\Aws\Api\DateTimeResult($processData['Expiration']);
                } catch (\Exception $e) {
                    return self::reject("credential_process returned invalid expiration");
                }
                $now = new \BWPS\SSU\Aws3\Aws\Api\DateTimeResult();
                if ($expiration < $now) {
            ...
            return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for(new \BWPS\SSU\Aws3\Aws\Credentials\Credentials($processData['AccessKeyId'], $processData['SecretAccessKey'], $processData['SessionToken'], $expires));
        };
    }
    /**
    ...
    private static function loadRoleProfile($profiles, $profileName, $filename, $stsClient, $config = [])
    {
        ...
        if (empty($stsClient)) {
            ...
            if (!empty($roleProfile['source_profile'])) {
                $sourceCredentials = call_user_func(\BWPS\SSU\Aws3\Aws\Credentials\CredentialProvider::ini($sourceProfileName, $filename, $config))->wait();
            } else {
                $sourceCredentials = self::getCredentialsFromSource($profileName, $filename);
            }
            $stsClient = new \BWPS\SSU\Aws3\Aws\Sts\StsClient(['credentials' => $sourceCredentials, 'region' => $sourceRegion, 'version' => '2011-06-15']);
        }
        $result = $stsClient->assumeRole(['RoleArn' => $roleArn, 'RoleSessionName' => $roleSessionName]);
        $credentials = $stsClient->createCredentials($result);
        return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for($credentials);
    }
    /**
    ...
    {
        $profileData = \BWPS\SSU\Aws3\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \BWPS\SSU\Aws3\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (file_exists($credFile)) {
            $profiles = \BWPS\SSU\Aws3\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }
        if (file_exists($configFile)) {
            $configProfileData = \BWPS\SSU\Aws3\Aws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
    ...
    public static function getCredentialsFromSource($profileName = '', $filename = '', $config = [])
    {
        ...
        switch ($credentialSource) {
            ...
            case 'EcsContainer':
                ...
                break;
            default:
                throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException("Invalid credential_source found in config file: {$credentialSource}. Valid inputs " . "include Environment, Ec2InstanceMetadata, and EcsContainer.");
        }
        ...
        return function () use($credentialsResult) {
            return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for($credentialsResult);
        };
    }
    private static function reject($msg)
    {
        return new \BWPS\SSU\Aws3\GuzzleHttp\Promise\RejectedPromise(new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException($msg));
    }
    /**
     * @param $filename
     * @return string
