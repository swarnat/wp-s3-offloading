...
use BWPS\SSU\Aws3\Aws\Exception\InvalidJsonException;
use BWPS\SSU\Aws3\Aws\Sdk;
use BWPS\SSU\Aws3\GuzzleHttp\Exception\TransferException;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->attempts = 0;
        $this->client = isset($config['client']) ? $config['client'] : \BWPS\SSU\Aws3\Aws\default_http_handler();
    }
    /**
    ...
    {
        return \BWPS\SSU\Aws3\GuzzleHttp\Promise\coroutine(function () {
            ...
            (yield new \BWPS\SSU\Aws3\Aws\Credentials\Credentials($result['AccessKeyId'], $result['SecretAccessKey'], $result['Token'], strtotime($result['Expiration'])));
        });
    }
    /**
     * @param string $url
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        if (strcasecmp($disabled, 'true') === 0) {
            throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException($this->createErrorMessage('EC2 metadata service access disabled'));
        }
        $fn = $this->client;
        $request = new \BWPS\SSU\Aws3\GuzzleHttp\Psr7\Request($method, self::SERVER_URI . $url);
        $userAgent = 'aws-sdk-php/' . \BWPS\SSU\Aws3\Aws\Sdk::VERSION;
        if (defined('HHVM_VERSION')) {
            $userAgent .= ' HHVM/' . HHVM_VERSION;
        }
        $userAgent .= ' ' . \BWPS\SSU\Aws3\Aws\default_user_agent();
        $request = $request->withHeader('User-Agent', $userAgent);
        foreach ($headers as $key => $value) {
            $request = $request->withHeader($key, $value);
        }
        return $fn($request, ['timeout' => $this->timeout])->then(function (\BWPS\SSU\Aws3\Psr\Http\Message\ResponseInterface $response) {
            ...
            throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException($this->createErrorMessage($msg));
        });
    }
    private function handleRetryableException(\Exception $e, $retryOptions, $message)
    {
        ...
        if ($isRetryable && $this->attempts < $this->retries) {
            ...
            throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException($message);
        }
    }
    ...
    private function decodeResult($response)
    {
        ...
        if (json_last_error() > 0) {
            throw new \BWPS\SSU\Aws3\Aws\Exception\InvalidJsonException();
        }
        if ($result['Code'] !== 'Success') {
            throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException('Unexpected instance profile ' . 'response code: ' . $result['Code']);
        }
        return $result;
    }
}
