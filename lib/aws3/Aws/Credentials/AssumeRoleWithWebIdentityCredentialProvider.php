...
class AssumeRoleWithWebIdentityCredentialProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['client'])) {
            ...
            $this->client = new \BWPS\SSU\Aws3\Aws\Sts\StsClient(['credentials' => false, 'region' => $region, 'version' => 'latest']);
        }
    }
    /**
                ...
                try {
                    ...
                    if (false === $token) {
                        ...
                        if (!is_readable($this->tokenFile)) {
                            throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException("Unreadable tokenfile at location {$this->tokenFile}");
                        }
                    ...
                    if (empty($token)) {
                        ...
                        throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException("InvalidIdentityToken from file: {$this->tokenFile}");
                    }
                } catch (\Exception $exception) {
                    throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException("Error reading WebIdentityTokenFile from " . $this->tokenFile, 0, $exception);
                }
                ...
                try {
                    ...
                    if ($e->getAwsErrorCode() == 'InvalidIdentityToken') {
                        if ($this->authenticationAttempts < $this->retries) {
                            ...
                            throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException("InvalidIdentityToken, retries exhausted");
                        }
                    } else {
                        throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException("Error assuming role from web identity credentials", 0, $e);
                    }
                } catch (\Exception $e) {
                    throw new \BWPS\SSU\Aws3\Aws\Exception\CredentialsException("Error retrieving web identity credentials: " . $e->getMessage() . " (" . $e->getCode() . ")");
                }
