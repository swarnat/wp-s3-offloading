...
use BWPS\SSU\Aws3\Aws\ResponseContainerInterface;
 ...
 * @internal
 */
abstract class AbstractMonitoringMiddleware implements \BWPS\SSU\Aws3\Aws\ClientSideMonitoring\MonitoringMiddlewareInterface
{
    ...
    protected $region;
    protected $service;
    protected static function getAwsExceptionHeader(\BWPS\SSU\Aws3\Aws\Exception\AwsException $e, $headerName)
    {
    ...
    protected static function getResultHeader(\BWPS\SSU\Aws3\Aws\ResultInterface $result, $headerName)
    {
    ...
    public function __invoke(\BWPS\SSU\Aws3\Aws\CommandInterface $cmd, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $g = function ($value) use($eventData, $enabled) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                return \BWPS\SSU\Aws3\GuzzleHttp\Promise\rejection_for($value);
            }
            return $value;
        };
        return \BWPS\SSU\Aws3\GuzzleHttp\Promise\promise_for($handler($cmd, $request))->then($g, $g);
    }
    private function getClientId()
    {
        return $this->unwrappedOptions()->getClientId();
    }
    private function getNewEvent(\BWPS\SSU\Aws3\Aws\CommandInterface $cmd, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $event = ['Api' => $cmd->getName(), 'ClientId' => $this->getClientId(), 'Region' => $this->getRegion(), 'Service' => $this->getService(), 'Timestamp' => (int) floor(microtime(true) * 1000), 'UserAgent' => substr($request->getHeaderLine('User-Agent') . ' ' . \BWPS\SSU\Aws3\Aws\default_user_agent(), 0, 256), 'Version' => 1];
        return $event;
    }
     ...
     * @return array
     */
    protected function populateRequestEventData(\BWPS\SSU\Aws3\Aws\CommandInterface $cmd, \BWPS\SSU\Aws3\Psr\Http\Message\RequestInterface $request, array $event)
    {
    ...
    private function unwrappedOptions()
    {
        if (!$this->options instanceof ConfigurationInterface) {
            try {
                $this->options = \BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::unwrap($this->options);
            } catch (\Exception $e) {
                // Errors unwrapping CSM config defaults to disabling it
                $this->options = new \BWPS\SSU\Aws3\Aws\ClientSideMonitoring\Configuration(false, \BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \BWPS\SSU\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT);
            }
        }
        return $this->options;
    }
