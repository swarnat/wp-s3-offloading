...
final class Env
{
    ...
    public static function search($expression, $data)
    {
        static $runtime;
        if (!$runtime) {
            $runtime = \BWPS\SSU\Aws3\JmesPath\Env::createRuntime();
        }
    ...
    public static function createRuntime()
    {
        switch ($compileDir = self::getEnvVariable(self::COMPILE_DIR)) {
            case false:
                return new \BWPS\SSU\Aws3\JmesPath\AstRuntime();
            case 'on':
                return new \BWPS\SSU\Aws3\JmesPath\CompilerRuntime();
            default:
                return new \BWPS\SSU\Aws3\JmesPath\CompilerRuntime($compileDir);
        }
    }
    /**
