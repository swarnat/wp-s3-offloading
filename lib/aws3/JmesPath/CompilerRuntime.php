...
class CompilerRuntime
{
     ...
     */
    public function __construct($dir = null, \BWPS\SSU\Aws3\JmesPath\Parser $parser = null)
    {
        $this->parser = $parser ?: new \BWPS\SSU\Aws3\JmesPath\Parser();
        $this->compiler = new \BWPS\SSU\Aws3\JmesPath\TreeCompiler();
        $dir = $dir ?: sys_get_temp_dir();
        if (!is_dir($dir) && !mkdir($dir, 0755, true)) {
            throw new \RuntimeException("Unable to create cache directory: {$dir}");
        }
        $this->cacheDir = realpath($dir);
        $this->interpreter = new \BWPS\SSU\Aws3\JmesPath\TreeInterpreter();
    }
    /**
