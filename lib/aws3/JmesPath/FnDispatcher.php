...
class FnDispatcher
{
    ...
    private function fn_avg(array $args)
    {
        ...
        $sum = $this->reduce('avg:0', $args[0], ['number'], function ($a, $b) {
            return \BWPS\SSU\Aws3\JmesPath\Utils::add($a, $b);
        });
    ...
    private function fn_sort(array $args)
    {
        ...
        $valid = ['string', 'number'];
        return \BWPS\SSU\Aws3\JmesPath\Utils::stableSort($args[0], function ($a, $b) use($valid) {
    ...
    private function fn_sort_by(array $args)
    {
        ...
        $expr = $args[1];
        $valid = ['string', 'number'];
        return \BWPS\SSU\Aws3\JmesPath\Utils::stableSort($args[0], function ($a, $b) use($expr, $valid) {
            $va = $expr($a);
            $vb = $expr($b);
    ...
    private function fn_type(array $args)
    {
        ...
        return \BWPS\SSU\Aws3\JmesPath\Utils::type($args[0]);
    }
    ...
    private function fn_to_number(array $args)
    {
        ...
        $value = $args[0];
        $type = \BWPS\SSU\Aws3\JmesPath\Utils::type($value);
        if ($type == 'number') {
            return $value;
    ...
    private function fn_to_array(array $args)
    {
        ...
        return \BWPS\SSU\Aws3\JmesPath\Utils::isArray($args[0]) ? $args[0] : [$args[0]];
    }
    ...
    {
        if ($types[0] == 'any' || in_array(\BWPS\SSU\Aws3\JmesPath\Utils::type($value), $types) || $value === [] && in_array('object', $types)) {
            return;
        }
        $msg = 'must be one of the following types: ' . implode(', ', $types) . '. ' . \BWPS\SSU\Aws3\JmesPath\Utils::type($value) . ' found';
    ...
    {
        $ta = \BWPS\SSU\Aws3\JmesPath\Utils::type($a);
        $tb = \BWPS\SSU\Aws3\JmesPath\Utils::type($b);
        if ($ta !== $tb) {
